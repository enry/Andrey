...
//void MD5Update (const char *inBuf, unsigned int inLen) {
...
QString MD5(QString in) {
    ...
    QByteArray tmpByteArray = in.toUtf8();
    const char* cString = tmpByteArray.constData();

    GetMD5(cString, in.length(), out_char);
    return QString(out_char);
}
